...
instance Show Digest
  -- show (Digest b) = appEndo (foldMap Endo $ showHex <$> B.unpack b) ""
                                                                          where
  show (Digest b) = concatMap hex $ B.unpack b

data ShaWork = ShaWork
  { numBytes :: Word64
...
doChunk current bs =
  ...
  where
    ...
    -- go i xs | traceShow ("i " <> (show $ i)) False = undefined
    go i _ | i == 0 && traceShow ("bs " <> (show $ B.length bs)) False = undefined
    go i xs | i == 0 && traceShow ("xs " <> (show $ length xs)) False = undefined
    go _ _ | traceShow ("sched " <> (show $ length schedule)) False = undefined
    go _ _ | trace ("sched " <> (T.pack $ intercalate "\n" $ bin <$> schedule)) False = undefined
    ...
    go _ _ = traceShow "whattafick" undefined

createMessageSchedule :: [Word32] -> [Word32]
createMessageSchedule prefix = go 48 $ reverse prefix
  where
    go :: Int -> [Word32] -> [Word32]
    go size ws | size == 0 = reverse ws
    go size ws' | otherwise =
        let ws = next : ws'
            w15 = ws !! 15
            w7 = ws !! 7
            w16 = ws !! 16
...
sha256finish work =
  let state = hashState work
      pending = pendingWork work
      bytes = numBytes work
      padding = sha_padding bytes
      result = doChunk state (pending <> padding)
   in Digest . B.pack $ concatMap w32_to_w8 result
